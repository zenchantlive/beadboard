...
describe('GET /api/runtime/agents', () => {
  ...
  it('returns canonical AgentState summary and instances from the shared runtime contract', async () => {
    ...
    assert.equal(payload.status.totalActive, 0);
    assert.equal(payload.status.instances.length, 0);
    assert.deepEqual(payload.liveWorkerIds, []);
    assert.equal(payload.agentStates.length, 1);
    assert.equal(payload.agentStates[0].status, 'failed');
  });
});
